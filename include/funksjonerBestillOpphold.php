...
function bekreftDatoer($dblink) {
    if (isset($_POST['bekreftDatoer'])) { 
        ...
        // sjekker at startDato ikke er før sluttDato
        if ($startDato >= $sluttDato) {
            echo "<br>".'<i style="color:red; position:absolute";"> StartDato må være før sluttDato! </i>'; 
        }
        else {
            // sjekker at vi har ledig bur disse datoene
            $burID = finnLedigBur($dblink,$startDato,$sluttDato);
            if ($burID<1) {
                echo "<br>".'<i style="color:red; position:absolute";"> Beklager! Vi har ingen ledige bur disse datoene! </i>'; 
            }
            // skjekker at ingen av disse hundene allerede har opphold i denne tidsperioden
            else if (harNoenAvHundeneAlleredeOpphold($dblink,$startDato,$sluttDato)) {
                echo "<br>".'<i style="color:red; position:absolute";"> Hund(ene) har allerede opphold i dette tidsrommet! </i>'; 
            }
            else {   
                $totalPris = totalPris($dblink,$startDato,$sluttDato);
                opprettBestillingSession($startDato, $sluttDato, $totalPris,$burID);
 ...
 **/  
function harNoenAvHundeneAlleredeOpphold($dblink,$startDato,$sluttDato) { 
    $alleredeOpphold = false;
    // valgteHunder
    $valgteHunder = $_SESSION['valgteHunder'];
    for ($i=0; $i<count($valgteHunder); $i++) {
        $hundID = $valgteHunder[$i];
        if ( harHundenAlleredeOpphold($dblink,$startDato,$sluttDato,$hundID) ) {
 ...
 *  @return boolean $alleredeOpphold  
 **/  
function harHundenAlleredeOpphold($dblink,$startDato,$sluttDato,$hundID) { 
    $alleredeOpphold = false;
 ...
 */ 
function opprettBestillingSession($startDato, $sluttDato, $totalPris, $burID) { 
    $bestilling = new Bestilling($startDato, $sluttDato, $totalPris, $burID);
